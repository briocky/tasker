...
export default function MyCategories() {
  ...
  useEffect(() => {
    getCategories(page, size).then((categories) => {
      setMyCategories(categories);
    });
  }, [page, size]);

  useEffect(() => {
    getSharedCategories(sharedCategoriesPage, size).then((categories) => {
    ...
    if (!myCategories) {
      ...
      return (
        <>
          ...
          <Box flex={'1'} component={'div'} display={'flex'} justifyContent={'center'} alignItems={'end'} mt={12}>
  ...
  return (
    <Container component="main" maxWidth="lg">
      <Box component={'div'} display={'flex'} justifyContent={'space-between'} alignItems={'center'} mb={1}>
        <Typography variant="h4" color="primary" gutterBottom mt={3} sx={{ textDecoration: 'underline' }}>
          My categories
        </Typography>
      ...
      {displayMyCategories()}
      <Typography variant="h4" color="primary" gutterBottom sx={{ textDecoration: 'underline' }} my={3}>
        Shared
      </Typography>
